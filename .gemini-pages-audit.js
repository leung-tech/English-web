...
expect(s2Connect?.grammar?.questions?.length === 16, 'S2 Connect standalone grammar requires 16 questions');
expect(s2Connect?.vocabulary?.items?.length === 18, 'S2 Connect standalone vocabulary requires 18 items');
...
console.log(JSON.stringify({
  ...
  info: [`Standalone S2 pages checked: ${expectedPages.length}`, 'Standalone S2 content items checked: 128'],
  errors
}, null, 2));
