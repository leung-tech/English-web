...
vm.runInContext(fs.readFileSync('senior/senior-extension-4.js', 'utf8'), context, { filename: 'senior-extension-4.js' });
...
if (!index.includes('senior-data.js') || !index.includes('senior-extension-2.js') || !index.includes('senior-extension-3.js') || !index.includes('senior-extension-4.js') || !index.includes('senior.js')) failures.push('senior assets are not loaded');
...
if (!app.includes('Original Paper 3 skills practice')) failures.push('Paper 3 original-practice boundary is missing');
if (!app.includes('No official Paper 3 score or writing-quality score')) failures.push('Paper 3 no-score boundary is missing');
...
const requirements = { s4:{ grammar:10, reading:12, writing:2, paper2:1, paper3:1, listening:6, oral:3 }, s5:{ grammar:10, reading:12, writing:2, paper2:1, paper3:1, listening:6, oral:3 }, s6:{ grammar:10, reading:12, writing:2, paper2:1, paper3:1, listening:6, oral:3 } };
const report = {};
...
const allPaper3 = (data.modules || []).filter((item) => item.skill === 'paper3').flatMap((item) => item.items || []);
if (!allPaper3.every((item) => item.audioScript?.length > 160 && item.sourcePack?.length >= 3 && item.noteItems?.length >= 6 && item.integrationPlan && item.responseMap?.length >= 4 && item.selfReview?.length >= 4)) failures.push('Paper 3 tools are missing listening input, source pack, notes or integration planning');
