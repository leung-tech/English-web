(() => {
  ...
  function bindTaskEvents() {
    ...
    if (next) next.onclick = () => { const current = module(); state.index = (state.index + 1) % current.items.length; state.selected = null; state.checked = false; state.showTranscript = false; state.paper3Answers = {}; state.paper3Checked = false; renderTask(); };
    ...
    document.querySelectorAll('[data-paper3-choice]').forEach((input) => input.onchange = () => { if (!state.paper3Checked) { state.paper3Answers[Number(input.dataset.paper3Choice)] = Number(input.value); renderTask(); } });
    const paper3Check = $('[data-paper3-check]');
    if (paper3Check) paper3Check.onclick = () => { const item = task(); if (!item || Object.keys(state.paper3Answers).length !== item.noteItems.length) return; state.paper3Checked = true; const accurate = item.noteItems.every((note, index) => state.paper3Answers[index] === note.answer); mark(module().id, accurate); renderTask(); renderProgress(); };
    const paper3Reset = $('[data-paper3-reset]');
    if (paper3Reset) paper3Reset.onclick = () => { state.paper3Answers = {}; state.paper3Checked = false; renderTask(); };
